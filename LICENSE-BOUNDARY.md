...
| 模型权重与数据集 | 不随**代码树**分发；以 Release 资产（tag `assets-v1`）按各自条款提供（YOLO11 系权重按 Ultralytics 许可框架的 AGPL-3.0 路径处理；DINOv3 衍生权重受 DINOv3 License 约束、**未纳入公开 Release**，再分发前需完成书面核验；数据集为平台数据，再分发待书面确认），均不适用项目 MIT |
